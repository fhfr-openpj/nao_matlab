...
while wb_robot_step(TIME_STEP) ~= -1
  ...
  subplot(2,4,3,'Parent',dashFig);
    ...
    if steps>50 && planState == "PLAN" && ~isempty(rangeMat)
      ...
      if ~isempty(xyHits)
          ...
          for s = 1:forwardBatch
              ...
              fprintf("  [%d]  pos = (%.2f, %.2f)  |  nLat = %+d  (ΔX = %.2f)\n", ...
    ...
    elseif planState == "MOVE" && ~isempty(planOut)
        ...
        mp         = movePlan(iterInPlan);     % 이번 스텝 계획
        ...
        robot_pos(2) = robot_pos(2) + stepFwd;     % y 갱신

        % ② 좌 / 우 -------------------------------------------------
        if mp.nLat < 0                      % ① 음수 = 왼쪽
            for s = 1:abs(mp.nLat)          %    ← 절댓값만큼 반복
                wbu_motion_play(SideStepLeft);
                while ~wbu_motion_is_over(SideStepLeft)
                    wb_robot_step(TIME_STEP);
                end
            end
        elseif mp.nLat > 0                  % ② 양수 = 오른쪽
            for s = 1:mp.nLat
        ...
        addpoints(hTrace,robot_pos(1),robot_pos(2));
            ...
            fprintf('[Fwd %2d/%d] idx=%d  tgtX=%.2f  ΔX=%.2f → nLat=%+d  pos=(%.2f,%.2f)\n',...
        iterInPlan,forwardBatch, mp.idx, mp.tgtX, mp.deltaX, mp.nLat,...
        ...
        if iterInPlan >= forwardBatch  % 4회 완료
            robot_pos  = [10,2];             % ★ 위치 초기화
            planState  = "PLAN";             % 새 궤적 다시 계획
            iterInPlan = 0;                  % 루프 카운터 리셋
            fwdCnt     = 0;
        end
    end
%%
